    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-4xl font-bold text-foreground mb-12">Promociones Especiales</h1>
          
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {promotions.map(promo => (
              <div key={promo.id} className="bg-secondary rounded-lg p-8 border-2 border-primary hover:border-accent transition-colors">
                <div className="text-5xl font-bold text-primary mb-4">
                  {promo.discount > 0 ? `${promo.discount}%` : '¡GRATIS!'}
                </div>
                <h2 className="text-2xl font-bold text-foreground mb-2">{promo.title}</h2>
                <p className="text-foreground/70 mb-4">{promo.description}</p>
                <p className="text-sm text-foreground/60">Válido hasta: {promo.validUntil}</p>
